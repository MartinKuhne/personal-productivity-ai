...
2. **The MCP stdio spawn
   ([`src/desktop/src/agent/lib/mcp/session.rs:1081`](../../src/desktop/src/agent/lib/mcp/session.rs),
   pre-fix)**
...
|---|---|---|
| `agent/lib/mcp/oauth/store.rs:344` | `icacls` to set token-store ACLs | One-shot |
| `agent/lib/mcp/manager.rs:2646` | `python` / `python3` / `py --version` discovery probe | One-shot at discovery |
...
- `cargo fmt --check src/agent/lib/mcp/session.rs` — clean.
