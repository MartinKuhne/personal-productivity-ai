...
- `src/desktop/src/integrations/mcp/` — MCP client protocol (manager, sessions, transports, OAuth flow)
...
| **V12** | **MCP server outputs return content verbatim to the LLM** | **High (new)** | `integrations/mcp/session.rs` (call path) | A malicious or compromised MCP server can return tool results that look like system instructions. Same class as V6 but the trust boundary is wider — MCP servers are third-party processes. |
