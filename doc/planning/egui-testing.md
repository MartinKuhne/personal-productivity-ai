...
8. **Q2's 5-pixel threshold — is that still the right number?** The egui 0.35 upgrade may have shifted default font metrics; a 5px diff on a wide FTWA table will hide a real "column 3 suddenly wraps an extra line" regression. **Open.** Lean: 3px for the FTWA table snapshot, 5px for the others, set per-harness. Decide in the P0-4 PR.

9. **Should the next 5 Tier 4 click tests (R-8) land in one PR or spread across multiple?** R-8's candidates are TOC row click, tab close, folder tree click, send button, batch button. None need an `AppHarness` (consistent with Q4's deferral). **Open.** Lean: one PR, because the helper from R-3 lands in the same PR and the next contributor benefits from seeing all 5 idioms at once.

10. **Tier 2 tautology policy (P1-5).** When a panel smoke test's only assertion is "did the call not panic", is the right answer (a) delete the test, (b) replace it with a text-content assertion per R-2, or (c) leave it as documentation-of-intent? **Open.** Lean: (b) for panels with stable, locatable strings; (a) for tests that are pure no-ops. Decide per-panel when the R-2 sweep lands.

11. **If `doc/planning/render-architecture.md` lands the 4-stage pipeline refactor, what tier does the new `MarkdownDoc` AST get tested at?** Almost certainly Tier 1, with the tests moving from `src/ui/render.rs` to `src/markdown/ast.rs`. The egui-testing strategy stays consistent across the refactor as long as Tier 1 is preserved. **Open; flag for the refactor PR.**
...
|---|---|---|---|
| **R-1** | **Highest** | Add `insta = "1.40"` to dev-deps; add `tests/common/snapshot.rs` helper that wraps `Harness::builder().with_size()` and `harness.snapshot(name)` with the 5px threshold from Q2; take 5–8 initial snapshots (full-markdown document, 6-col FTWA table, empty-cell table, bold-cell table, heading-scroll, move-file modal, bottom-panel with command input); add `cargo insta pending-snapshots` CI step. | 1.5 days |
| **R-2** | High | Replace ~12 tautological panel smoke tests in `panels/{top,bottom,left,center}.rs` and `modals.rs` with text-content assertions using a `tests/common/egui_assert.rs` helper (`assert_text_contains(shapes, "Indexing finished")` etc.). Use `crate::ui::strings::*` constants rather than literals (resolves P1-7 in the same pass). | 0.5 day |
...
| **R-6** | Deferred to R-1 | The 5px threshold and "no `#[cfg(target_os)]`" decisions are encoded in the R-1 helper. No separate work. | 0 |
...
| **R-8** | Medium (deferred to R-3) | Five Tier 4 click tests: TOC row click → `scroll_to_id`, tab close button, folder tree click → `selected_file`, send button → `submit_prompt`, batch button → `batch_dialog_open`. Lands in one PR with R-3's helper. | 0.5 day |
