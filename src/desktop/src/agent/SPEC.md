...
* [AGENT-014] Active Directory Context: The directory context for the AI prompt shall be the parent directory of the file selected in the tabs panel when one is selected, otherwise the last directory the user selected from the directory tree. The system shall send the full virtual path of the directory context (see [`src/app/vfs/SPEC.md`](../app/vfs/SPEC.md), VFS-004) with the system prompt.
...
* [AGENT-026] Empty Tab Workspace File Context Suppression: When the user closes all open tabs and then sends an AI prompt, the system shall not pass file-level context (the active file or the selected-files set) to the LLM system prompt. The directory context (AGENT-014) is still handed over: it falls back to the last directory the user selected from the directory tree.
