...
pub fn tool_search_calendar(config: &AppConfig, keyword: &str) -> String {
...
pub fn tool_get_calendar(config: &AppConfig, start: &str, end: &str) -> String {
    let mut all_results = Vec::new();
    for (name, client) in &config.jmap_clients {
...
pub fn tool_get_calendar_item(config: &AppConfig, id: &str) -> String {
    let mut all_results = Vec::new();
    for (name, client) in &config.jmap_clients {
        let (api_url, token, accs) = match get_jmap_session(client) { Ok(s) => s, Err(_) => continue };
        let account_id = get_account_id(&accs, "urn:ietf:params:jmap:calendars");
    ...
    if all_results.is_empty() { "Event not found in any client or no clients configured.".to_string() } else { all_results.join("\n\n") }
}

pub fn tool_add_calendar_item(config: &AppConfig, item_json: &str) -> String {
    let mut all_results = Vec::new();
    for (name, client) in &config.jmap_clients {
        let (api_url, token, accs) = match get_jmap_session(client) { 
            Ok(s) => s, 
        ...
        let item: Value = match serde_json::from_str(item_json) {
            Ok(i) => i, Err(e) => return format!("Error: Invalid JSON: {}", e),
        };
...
pub fn tool_update_calendar_item(config: &AppConfig, id: &str, update_json: &str) -> String {
    let mut all_results = Vec::new();
    for (name, client) in &config.jmap_clients {
        let (api_url, token, accs) = match get_jmap_session(client) { 
            Ok(s) => s, 
        ...
        let update: Value = match serde_json::from_str(update_json) {
            Ok(u) => u, Err(e) => return format!("Error: Invalid JSON: {}", e),
        };
...
pub fn tool_delete_calendar_item(config: &AppConfig, id: &str) -> String {
    let mut all_results = Vec::new();
    for (name, client) in &config.jmap_clients {
        let (api_url, token, accs) = match get_jmap_session(client) { 
            Ok(s) => s, 
