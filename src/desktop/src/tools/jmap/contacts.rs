...
pub fn tool_search_contact(config: &AppConfig, keyword: &str) -> String {
...
pub fn tool_get_contact(config: &AppConfig, id: &str) -> String {
    let mut all_results = Vec::new();
    for (name, client) in &config.jmap_clients {
        let (api_url, token, accs) = match get_jmap_session(client) { 
            Ok(s) => s, 
    ...
    if all_results.is_empty() { "Contact not found in any client or no clients configured.".to_string() } else { all_results.join("\n\n") }
}

pub fn tool_add_contact(config: &AppConfig, contact_json: &str) -> String {
    let mut all_results = Vec::new();
