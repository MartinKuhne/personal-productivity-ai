...
* [REQ-218] Paragraph and Heading Word-Wrap: The FastMD Viewer shall word-wrap paragraph text and heading text within the available viewport width. A single styled text span (including a span containing an unbreakable token longer than the available width) shall be broken at character boundaries so it fits the available width and does not overflow the panel. Code blocks (REQ-216) and table cells (REQ-208..211b) already word-wrap; this requirement extends the same behaviour to the regular paragraph and heading paths. Regression coverage lives in `src/ui/render.rs::e2e_tests` (`test_paragraph_long_unbreakable_token_wraps`, `test_paragraph_long_sentence_wraps`, `test_long_heading_wraps`).
