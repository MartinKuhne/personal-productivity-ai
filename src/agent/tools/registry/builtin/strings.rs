...
#[allow(dead_code)]
pub const CURSOR_PAGINATION_CANONICAL_DESCRIPTION: &str = "This tool uses cursor pages. Omit `cursor` on the first call. Give the `cursor` back unchanged to get the next page. When `cursor` is given, other search parameters are ignored. The last page has a `hint` field and no `cursor`.";
...
pub const FIELD_CURSOR_DESCRIPTION: &str = "Omit this field on the first call. To get the next page, give back the `cursor` value unchanged. When `cursor` is given, other search parameters are ignored.";
