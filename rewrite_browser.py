...
def rewrite_builtin_browser(filepath):
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()

    # We need to replace all uses of:
    # 1. `ctx.extensions.get::<crate::app::session::BrowserSession>().unwrap()` -> `ctx.extensions.get::<crate::agent::tools::browser::BrowserExt>().unwrap().0`
    # 2. `handle.page.goto(&input.url, None).await` -> `ext.navigate(&input.url)`
    # ... and so on.
    
    # Actually, it's easier to just do regex or manual replacements for the blocks.
    # Let's write the whole file manually.

    new_content = """//! LLM tool wrappers for the headless browser (`BRWS-001..008`).
//!
//! Each tool's `Tool::execute` runs the underlying browser extension methods.
//! Mutating tools trigger a `save_storage()` so cookies / local
...
/// Convert any error string into a Tool error string.
...
fn execute_browser_navigate(
    ...
    let browser = ext(ctx)?;
    let (final_url, title) = browser.navigate(&input.url).map_err(err)?;
...
fn execute_browser_get_page_state(
    ...
    let browser = ext(ctx)?;
    let (url, title, elements_json, total) = browser.get_page_state().map_err(err)?;
...
fn execute_browser_click(
    ...
    let input: dtos::BrowserClickInput =
        serde_json::from_str(args).map_err(|e| err(format!("Invalid args: {}", e)))?;
    let browser = ext(ctx)?;
    browser.click(&input.selector).map_err(err)?;
    let _ = browser.save_storage();
...
fn execute_browser_press_key(
    ...
    let input: dtos::BrowserPressKeyInput =
        serde_json::from_str(args).map_err(|e| err(format!("Invalid args: {}", e)))?;
    let browser = ext(ctx)?;
    browser.press_key(&input.key).map_err(err)?;
    let _ = browser.save_storage();
...
fn execute_browser_evaluate_js(
    ...
    let input: dtos::BrowserEvaluateJsInput =
        serde_json::from_str(args).map_err(|e| err(format!("Invalid args: {}", e)))?;
    let browser = ext(ctx)?;
    let value = browser.evaluate_js(&input.script).map_err(err)?;
    let _ = browser.save_storage();
...
if __name__ == "__main__":
    rewrite_builtin_browser("C:/Users/mkuhn/src/ppai/src/desktop/src/agent/tools/registry/builtin/browser.rs")
