...
- [x] T012 [US1] Return `Vec<ToolSideEffect>` from `ToolExecutor::execute_all` in `src/desktop/src/agent/tool_executor.rs`; drop `tx_gui` parameter; delete `notify_file_creations` function (lines 160-215) (migration step 4, FR-006)
- [x] T013 [US1] Publish `AgentEvent::ToolSideEffect` for each side effect from agent loop in `src/desktop/src/agent/agent_impl.rs` after `execute_all` returns (migration step 4)
