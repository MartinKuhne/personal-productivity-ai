...
- [ ] T010 [US1] Move `response_formatter.rs` to `src/desktop/src/ui/render/agent_render.rs` — relocate `format_tool_call_message`, `format_tool_result_message`, `format_delegate_tool_call_message`, `split_thinking_and_content`; update all import sites (migration step 3, RUST-055)
